...
# dep_var_string: a string of the name of the dependent variable (must match the name in the dataset)
# data: the dataset
# nrep: number of bootstrap replicates
# free_indices: the indices revealing which coefficients will be unpenalized. see gamlr for more detail
# gamma_vec: the values of gamma we want to cross validate over
...
two_stage_gamma_BOLASSO <- function(x, y, dep_var_string, data, nrep = 100, free_indices = NULL, gamma_vec = c(0,1,10), thresh = 0.9,  
  ...
  vars <- vars[which(vars != dep_var_string)]
  
  # we create an R regression formula
  vars_form <- paste(vars, collapse = " + ")
  reg_formula <- as.formula(paste(dep_var_string, " ~", vars_form))
